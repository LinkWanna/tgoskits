...
/// 扩展控制（Extended Control）。
///
/// `value` 联合体与 Linux `struct v4l2_ext_control` 中的匿名联合体对应
/// （64 位目标上为 8 字节）；未展开复合类型指针成员 — 需要时自行
/// 对 `value.ptr` 进行类型转换。
#[repr(C)]
#[derive(Debug, Clone, Copy)]
