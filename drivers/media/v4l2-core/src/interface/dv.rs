//! DV timings（数字视频时序）结构 — 对应 Linux `v4l2_bt_timings`、
//! `v4l2_dv_timings`、`v4l2_enum_dv_timings`、`v4l2_dv_timings_cap`。
//!
//! DV timings 是现代 HDMI/DP 接口的时序描述（取代模拟电视 STD），
//! 属于当前活跃接口。
...
/// BT.656/BT.1120 时序数据（packed，对齐 Linux 布局）。
#[repr(C, packed)]
...
/// DV timings 载荷联合 — 对应 C 匿名联合体（bt 或保留字段）。
#[repr(C)]
#[derive(Clone, Copy)]
...
/// DV 时序 — 对应 `struct v4l2_dv_timings`（packed，对齐 Linux 布局）。
#[repr(C, packed)]
#[derive(Debug, Clone, Copy)]
...
/// BT.656/BT.1120 时序能力（packed，对齐 Linux 布局）。
#[repr(C, packed)]
#[derive(Debug, Clone, Copy)]
...
/// DV 时序能力载荷联合 — 对应 C 匿名联合体（bt 或原始数据）。
#[repr(C)]
#[derive(Clone, Copy)]
...
/// DV 时序能力 — 对应 `struct v4l2_dv_timings_cap`。
#[repr(C)]
#[derive(Debug, Clone, Copy)]
