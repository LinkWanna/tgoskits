//! 调试寄存器结构 — 对应 Linux `v4l2_dbg_register`、`v4l2_dbg_chip_info`。
//!
//! 这些 ioctl 仅供调试（`CONFIG_VIDEO_ADV_DEBUG`，需要 root 权限），
//! 属于遗留接口，应用代码不应使用。
...
/// 芯片匹配信息的值联合 — 对应 C 匿名联合体（addr 或 name）。
#[repr(C)]
#[derive(Clone, Copy)]
