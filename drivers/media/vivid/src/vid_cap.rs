...
impl IoctlOps for VividCapture {
    ...
    fn dqbuf(&mut self, buf: &mut Buffer) -> v4l2_core::Result<()> {
        ...
        let (plane_ptr, plane_size, seq, timestamp, timestamp_flags) = match &vb {
            Some(vb) => match vb.planes.first() {
                Some(p) => (
                    Some(p.as_ptr()),
                    Some(p.length as usize),
                    vb.sequence,
                    vb.timestamp,
                    vb.timestamp_flags,
                ),
                None => (None, None, vb.sequence, vb.timestamp, vb.timestamp_flags),
            },
            None => (None, None, 0, 0, 0),
        };

        if let (Some(ptr), Some(size)) = (plane_ptr, plane_size) {
    ...
    fn streamon(&mut self, _ty: BufType) -> v4l2_core::Result<()> {
        ...
        // 用测试图案预填充已入队的缓冲（vivid 同步填充）：直接获取 ActiveFrame 句柄
        let pat = self.state.pattern_index();
        let tpg_fmt = self.state.fmt_tpg();
        let (w, h) = self.state.fmt_dimensions();
        let (_, _, sizeimage) = self.state.fmt_dimensions_and_size();

        while let Some(mut frame) = q.acquire() {
            let buf = frame.as_mut_slice();
            let to_fill = &mut buf[..sizeimage as usize];
            crate::tpg::fill_buffer(pat, tpg_fmt, w, h, 0, to_fill);
            q.commit(frame, sizeimage)?;
        }
