...
pub fn serialize_stream_control(ctrl: &StreamControl) -> Vec<u8> {
...
/// 解析 StreamControl 响应
pub fn parse_stream_control(data: &[u8]) -> Result<StreamControl, USBError> {
    if data.len() < 26 {
...
/// 单次遍历快照 blob 得到的 UVC 设备初始化信息。
#[derive(Debug)]
pub(crate) struct UvcDeviceConfig {
    /// VideoControl 接口号（UVC 控件请求的 wIndex）。
    pub vc_iface_num: u8,
    /// VideoStreaming 接口号（VS 控件请求的 wIndex）。
    pub vs_iface_num: u8,
    /// VS 接口的备用设置（ISO IN 端点，流传输选路用）。
    pub alt_settings: Vec<AlternateSetting>,
    /// VS 接口支持的格式（Format/Frame 描述符）。
    pub formats: Vec<VideoFormat>,
    /// VC 接口单元（Camera Terminal / Processing Unit，控件注册用）。
    pub vc_units: VcUnits,
}

/// 单次遍历设备快照描述符 blob，完成 UVC 设备初始化所需的全部分析。
///
/// `blob` 布局与 usbfs 快照 `descriptor_blob` 一致：18 字节设备描述符 +
/// 配置描述符。UVC 设备有且仅有一个 VC 与一个 VS 接口，一次遍历同时确定：
///
/// 1. VC / VS 接口号（class 0x0E 的 VideoControl(0x01) / VideoStreaming(0x02)）；
/// 2. VS 接口的 AlternateSetting（ISO IN 端点：alt/mps/每微帧包数/间隔）；
/// 3. VS 接口格式（Format/Frame 描述符 → [`VideoFormat`]）与 VC 接口单元
///    （Camera Terminal / Processing Unit → [`VcUnits`]，控件注册用）。
///
/// 调用方解析完成后无需再保留 blob；解析失败（无配置描述符 / 缺 VC 或 VS
/// 接口 / 描述符损坏）返回错误。
pub(crate) fn parse_uvc_device(blob: &[u8]) -> Result<UvcDeviceConfig, USBError> {
    ...
    // 当前所在接口（接口描述符更新；端点/CS 描述符归属其所在接口）。
    // (bInterfaceNumber, bAlternateSetting, bInterfaceClass, bInterfaceSubClass,
    //  bInterfaceProtocol)
    let mut cur_iface: Option<(u8, u8, u8, u8, u8)> = None;
    // 当前 VS 格式（Format 描述符更新；Frame 描述符归属其所在格式）。
    let mut cur_format: Option<(u8, VideoFormatType)> = None;

    // 跳过 18 字节设备描述符（bLength=18, type=0x01）。
    let mut pos = 18usize;
    while pos + 2 <= blob.len() {
        ...
        match DescriptorType::from(descriptor_type) {
            DescriptorType::Interface if length >= 9 => {
                ...
                if class == 0x0E {
                    // UVC 设备有且仅有一个 VC 与一个 VS 接口，直接记录。
            ...
            DescriptorType::Endpoint if length >= 7 => {
                // 所选 VS 接口的 ISO IN 端点 → 一条备用设置。
                if let Some((number, alternate, class, subclass, protocol)) = cur_iface
                    && class == 0x0E
                    && InterfaceSubclass::from(subclass) == InterfaceSubclass::VideoStreaming
                    && protocol == 0x00
                    && vs_iface_num == Some(number)
                    && desc[3] & 0x03 == 0x01 // bmAttributes: isochronous
                    && desc[2] & 0x80 != 0
                // bEndpointAddress: IN
                {
...
/// 解析未压缩格式类型（仅返回格式类型，不生成 VideoFormat）
fn parse_uncompressed_format_type(data: &[u8]) -> Result<UncompressedFormat, USBError> {
    match DescriptorParser::new().parse_uncompressed_format(data) {
        Ok(desc) => {
            // 根据 GUID 确定格式类型
            let format_type = if desc.guid == crate::descriptors::format_guids::YUY2 {
                debug!("Detected YUY2 format");
                UncompressedFormat::Yuy2
            } else if desc.guid == crate::descriptors::format_guids::NV12 {
                debug!("Detected NV12 format");
                UncompressedFormat::Nv12
            } else if desc.guid == crate::descriptors::format_guids::RGB24 {
...
pub(crate) mod test_util {
    ...
    /// 构造 UVC 设备快照 blob：18 字节设备描述符 + 一个配置描述符。
    ///
    /// 配置含：VC 接口 0（Camera Terminal + Processing Unit）、非 UVC HID
    /// 接口 1（干扰项）、VS 接口 3（alt 0 带 YUY2 640x480@30 格式，
    /// alt 1/alt 2 各带一个 ISO IN 端点）。
    pub(crate) fn build_uvc_blob(vc_subclass: u8, vs_subclass: u8) -> Vec<u8> {
        ...
        // VC 接口 0 + VC 类特定描述符（Camera Terminal / Processing Unit）。
...
mod tests {
    ...
    fn parse_uvc_device_single_pass_extracts_all() {
        ...
        // 备用设置：同接口号 3 的 alt 1/alt 2 端点都收集
    ...
    #[test]
    fn parse_uvc_device_missing_vc_errors() {
        let blob = build_uvc_blob(0x00, 0x02);
        let err = parse_uvc_device(&blob).unwrap_err();
        assert!(err.to_string().contains("VideoControl"));
    }

    #[test]
    fn parse_uvc_device_missing_vs_errors() {
        let blob = build_uvc_blob(0x01, 0x00);
        let err = parse_uvc_device(&blob).unwrap_err();
        assert!(err.to_string().contains("VideoStreaming"));
    }

    #[test]
    fn parse_uvc_device_truncated_blob_errors() {
        let blob = build_uvc_blob(0x01, 0x02);
        assert!(parse_uvc_device(&blob[..20]).is_err());
    }
