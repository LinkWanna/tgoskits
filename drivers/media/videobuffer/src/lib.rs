...
mod tests {
    ...
    fn rejected_buffer_done_does_not_wake() {
        ...
        // 索引越界 → acquire 返回 None，无唤醒。
        assert!(q.acquire_for(9).is_none());
        assert_eq!(count.load(Ordering::SeqCst), 0);
        // 重复 acquire 同一缓冲应互斥，第二次为 None
        let _first = q.acquire().unwrap();
    ...
    /// acquire_for 精确获取指定索引。
    #[test]
    fn driver_acquire_for_specific_index() {
        let q = Vb2Queue::new(TestAlloc::default(), 2, 4);
        q.reqbufs(2, &[4096]).unwrap();
        q.qbuf(0).unwrap();
        q.qbuf(1).unwrap();
        q.streamon().unwrap();
        let frame = q.acquire_for(1).expect("acquire_for 1");
        assert_eq!(frame.index(), 1);
        q.commit(frame, 512).unwrap();
        assert!(q.is_readable());
    }
}
